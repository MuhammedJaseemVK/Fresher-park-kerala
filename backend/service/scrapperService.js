  ...
  try {
    ...
    const fresherJobs = await page.evaluate((jobSelector) => {
      const isAFresherJob = (title) => {
        ...
        const fresherJobKeywordMatched = fresherJobKeywords.some((keyword) =>
          jobTitle.includes(keyword)
        );
        if (pattern.test(jobTitle) || fresherJobKeywordMatched) {
          return true;
        } else {
          return false;
        }
        ...
        if (isAFresherJob(jobTitle)) {
          jobs.push({
            ...
            jobDeadline: jobDeadline
              ? getDeadlineDate(jobDeadline.innerText)
              : "unknown",
...
const scrapTechnoparkJobs = async () => {
  try {
    const firstPageData = await axios.get(
      "https://technopark.org/api/paginated-jobs?page=1&search=&type="
    );
    ...
    for (
      ...
      remainingPageJobsRequests.push(
        axios.get(
          `https://technopark.org/api/paginated-jobs?page=${currentPageNumber}&search=&type=`
        )
      );
    }
    ...
    const allJobsData = [
      ...firstPageJobsData,
      ...remainingPageJobsData
        .map((jobDataPerPage) => jobDataPerPage.data.data)
        .flat(),
    ];
    const fresherJobs = allJobsData.filter((job) =>
      isAFresherJob(job.job_title)
    );
    const transfomedFresherJobs = fresherJobs.map((job) => ({
      ...
      jobDeadline: job.closing_date
        ? getDeadlineDate(job.closing_date)
        : "unknown",
